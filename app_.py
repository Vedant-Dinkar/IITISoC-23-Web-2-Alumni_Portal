...
dab = client["Alumni"]
fs = GridFS(dab, collection="images")
...
def get_profile():
    ...
    '''image_path = request.form['picture']

    with open(image_path, "rb") as image_file:
        image_data = image_file.read()

    image_id = fs.put(image_data, filename="Picture.png")'''
    ...
    # Add the image_id to the profile information dictionary
    extracted_info["picture"] = f'/profile/image/{image_id}'''
...
@login_required_routes(login_required_routes_list)
def profile():
    '''image_id = extracted_info["image_id"]
